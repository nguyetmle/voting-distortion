...
class VoteResult3D:
    ...
    def getScores(self):
        ...
        for voter in self.voters:
            ...
            for candidate in self.candidates:
                distance = math.sqrt((voter.x - candidate.x) ** 2 + (voter.y - candidate.y) ** 2 + (voter.z - candidate.y)**2)
...
def main():
    test = VoteResult3D(10, 3, "3D", "normal")
    print(test.plurality())
    print(test.distortion(test.plurality()))
    print(test.majorityCheck(test.plurality()))
