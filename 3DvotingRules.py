...
def writeOut(voters,candidates,normal=True,errorList=False,winners=False,criteria=False,distortions=False,stvTie=False):
    n = len(voters)
    m = len(candidates)
    inputfile = open("3D_metricTests.txt","r+",encoding='UTF-8')
    file = inputfile.read()
    lines = file.split("\n")
    testNum = 0
    for line in lines:
    ...
    if normal:
        ...
        cCoords = []
        for can in candidates:
            coords = str((can.x,can.y,can.z))
            cCoords.append(coords)
        for voter in voters:
            coords = str((voter.x,voter.y,can.z))
...
def main():
    ...
    for i in range(n):
        ...
        z = round(random.normal(50, 20), 1)
        voter = SVoter(x, y, z, i)
        voters.append(voter)
    ...
    pVetoSumDistance = 0
    for voter in voters:
        pDistance = math.sqrt((voter.x - pWinner.x) ** 2 + (voter.y - pWinner.y) ** 2 + (voter.z - pWinner.z) ** 2)
        pSumDistance += pDistance
        bDistance = math.sqrt((voter.x - bWinner.x) ** 2 + (voter.y - bWinner.y) ** 2 + (voter.z - bWinner.z) ** 2)
        bSumDistance += bDistance
        cDistance = math.sqrt((voter.x - cWinner.x) ** 2 + (voter.y - cWinner.y) ** 2 + (voter.z - cWinner.z) ** 2)
        cSumDistance += cDistance
        pVetoDistance = math.sqrt((voter.x - pVetoWinner.x) ** 2 + (voter.y - pVetoWinner.y) ** 2 + (voter.z - pVetoWinner.z) **2)
        pVetoSumDistance += pVetoDistance
        if stvWinner:
            stvDistance = math.sqrt((voter.x - stvWinner.x) ** 2 + (voter.y - stvWinner.y) ** 2 + (voter.z - stvWinner.z) ** 2)
            stvSumDistance += stvDistance
        if starWinner:
            starDistance = math.sqrt((voter.x - starWinner.x) ** 2 + (voter.y - starWinner.y) ** 2 + (voter.z - starWinner.z) ** 2)
    ...
    test = [[OPTcandidate.id,pWinner.id,pDistortion,bWinner.id,bDistortion,cWinner.id,cDistortion,
             stvResult,stvDistortion,starResult,starDistortion,pVetoWinner.id,pVetoDistortion,rMajResult,rConResult,sMajResult,sConResult,n,m,3]]
