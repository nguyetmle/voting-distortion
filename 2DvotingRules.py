...
def writeOut(voters,candidates,normal=True,errorList=False,winners=False,criteria=False,distortions=False,stvTie=False):
    n = len(voters)
    m = len(candidates)
    inputfile = open("2D_metricTests.txt","r+",encoding='UTF-8')
    file = inputfile.read()
    lines = file.split("\n")
    testNum = 0
    for line in lines:
    ...
    if normal:
        ...
        cCoords = []
        for can in candidates:
            coords = str((can.x,can.y))
            cCoords.append(coords)
        for voter in voters:
            coords = str((voter.x,voter.y))
...
def plot(currentTest, voters, candidates, winners,fails):
    ...
    plt.scatter([optX],[optY],c = "green",s = 200,label = "Optimal")
    plt.scatter([pluX],[pluY],c = "red",s = 175,label = "Plurality")
    plt.scatter([borX],[borY],c = "yellow",s = 150,label = "Borda")
    plt.scatter([copX],[copY],c = "orange",s = 125,label = "Copeland")
    if winners[4]:
        plt.scatter([stvX],[stvY],c = "pink",s = 100,label = "STV")
    if winners[5]:
        plt.scatter([staX],[staY],c = "black",s = 75,label = "STAR")
    plt.scatter([pVetX],[pVetY],c = "white",s = 50,label = "Plurality-Veto")
...
def main():
    ...
    pVetoSumDistance = 0
    for voter in voters:
        pDistance = math.sqrt((voter.x - pWinner.x) ** 2 + (voter.y - pWinner.y) ** 2)
        pSumDistance += pDistance
        bDistance = math.sqrt((voter.x - bWinner.x) ** 2 + (voter.y - bWinner.y) ** 2)
        bSumDistance += bDistance
        cDistance = math.sqrt((voter.x - cWinner.x) ** 2 + (voter.y - cWinner.y) ** 2)
        cSumDistance += cDistance
        pVetoDistance = math.sqrt((voter.x - pVetoWinner.x) ** 2 + (voter.y - pVetoWinner.y) ** 2)
        pVetoSumDistance += pVetoDistance
        if stvWinner:
            stvDistance = math.sqrt((voter.x - stvWinner.x) ** 2 + (voter.y - stvWinner.y) ** 2)
            stvSumDistance += stvDistance
        if starWinner:
            starDistance = math.sqrt((voter.x - starWinner.x) ** 2 + (voter.y - starWinner.y) ** 2)
    ...
    test = [[OPTcandidate.id,pWinner.id,pDistortion,bWinner.id,bDistortion,cWinner.id,cDistortion,
             stvResult,stvDistortion,starResult,starDistortion,pVetoWinner.id,pVetoDistortion,rMajResult,rConResult,sMajResult,sConResult,n,m,2]]
